...
export default function Catalog() {
  ...
  // Перестановки / Юр. услуги
  const labels = { moving: 'Перестановки', legal: 'Юридические услуги' };
  const icons = { moving: '📦', legal: '⚖️' };
  return (
    <div>
      <BackButton onClick={() => setScreen('main')} title={labels[screen]} />
      <div className="flex flex-col items-center justify-center pt-16 px-8">
        <span style={{ color: '#F5E642' }} className="text-5xl mb-6">{icons[screen]}</span>
        <p className="text-white font-semibold text-lg mb-2">{labels[screen]}</p>
